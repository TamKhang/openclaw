...
export type PluginHookMessageContext = {
  ...
  /**
   * Trusted, internal-only outbound authorization marker. Populated only by
   * channel feature code that has already performed exact owner/target
   * validation. Never derived from message text, prompt text, quoted content,
   * or model output. Ordinary group outbound sends omit this field.
   */
   ...
   * non-model command/system responses. Never derived from message text,
   * model output, tool absence, or footer text.
   */
  provenanceExempt?: boolean;
};

/**
 * Trusted, single-use WhatsApp group outbound authorization permit.
 *
 * Carried by the outboundGroupReplyAuthorization message-hook field for both
 * authorization classes. Minted exclusively by trusted channel feature code
 * from actual trusted owner state; never derived from message text, prompt
 * text, quoted content, or model output.
 */
export type PluginHookOutboundGroupReplyAuthorization = {
  /** Authorization class. Only these two are admitted by the gate. */
  authorizationClass: "owner_explicit_send" | "delegated_group_reply";
  /** Gate policy version this permit was minted under. */
  policyVersion: 1;
  ...
  expiresAt: number;
  maxSends: 1;
  /** delegated_group_reply capability discriminator (absent for owner_explicit_send). */
  capability?: "whatsapp.group.reply_once";
  /** Exact owner trigger message ID that created the authorization. */
  ownerTriggerMessageId?: string;
  /** Exact quoted target message ID. */
  quotedMessageId?: string;
  /** Exact quoted participant identity. */
