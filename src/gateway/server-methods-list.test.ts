...
describe("listGatewayMethods", () => {
  ...
  it("preserves the legacy advertised method order", () => {
    ...
    expect(methods.indexOf("projection.read")).toBe(methods.indexOf("plugins.inspect") + 1);
    expect(methods.indexOf("projection.status")).toBe(methods.indexOf("projection.read") + 1);
  });
