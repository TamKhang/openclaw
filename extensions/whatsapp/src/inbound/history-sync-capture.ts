// WhatsApp passive history sync capture normalizes Baileys `messaging-history.set`
// group payloads into observation-only events. It never requests history, starts a
// turn, invokes a model, or sends outbound traffic.
...
/** Attaches only a passive listener; callers decide whether capture is enabled. */
  ...
  const listener = (payload: unknown) => {
    ...
    for (const message of messages) {
      ...
      const normalized = normalizeWhatsAppPassiveHistoryMessage({
        accountId: params.accountId,
        message,
        ...(typeof groupId === "string" ? { groupSubject: params.groupSubjectFor(groupId) } : {}),
      });
      if (normalized) {
